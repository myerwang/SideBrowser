export interface EmbeddingCheckResult {
  ...
  readonly blockedBy?: 'x-frame-options' | 'csp-frame-ancestors';
  readonly reason?: string;
  readonly warning?: string;
}

export async function inspectEmbeddingSupport(url: string): Promise<EmbeddingCheckResult> {
  try {
...
async function requestForEmbeddingInspection(
  url: string,
  method: 'GET' | 'HEAD'
): Promise<Response> {
  return fetch(url, {
...
export function inspectEmbeddingHeaders(headers: Headers): EmbeddingCheckResult | undefined {
...
function extractFrameAncestors(csp: string): string | undefined {
  const directive = csp
    .split(';')
    .map((part) => part.trim())
